...
/**
 * Filesystem discovery for the Alloy agent and the OBS runtime its recorder
 * lane loads. Pure lookups with no recording state.
 */

export function sidecarExecutablePath(): string {
  const executable = "alloy-agent.exe"
  if (app.isPackaged) return join(process.resourcesPath, "agent", executable)
  return join(app.getAppPath(), "..", "recorder", "dist", "agent", executable)
}
