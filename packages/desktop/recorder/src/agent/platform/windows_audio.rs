...
use windows_sys::Win32::{
    ...
    Foundation::{CloseHandle, HGLOBAL, HWND, RECT},
    Graphics::{
        Gdi::{
            GetMonitorInfoA, GetMonitorInfoW, MonitorFromWindow, MONITORINFO, MONITORINFOEXA,
    ...
    Media::Audio::{eRender, AudioSessionStateActive, AudioSessionStateInactive},
    Security::Cryptography::{CryptBinaryToStringW, CRYPT_STRING_BASE64, CRYPT_STRING_NOCRLF},
    Storage::FileSystem::{GetFileVersionInfoSizeW, GetFileVersionInfoW, VerQueryValueW},
    System::{
        Com::StructuredStorage::{CreateStreamOnHGlobal, GetHGlobalFromStream},
        Com::{CLSCTX_ALL, STATFLAG_NONAME, STATSTG},
    ...
    UI::{
        ...
        WindowsAndMessaging::{
            DestroyIcon, GetClassNameW, GetClientRect, GetShellWindow, GetWindowLongPtrW,
            GetWindowRect, GetWindowTextW, IsWindow, IsWindowVisible, GWL_EXSTYLE, HICON,
...
use super::{
    com::{
        active_audio_endpoint_devices, com_vtbl, create_mm_device_enumerator, initialize_com,
        query_interface, release_com, string_from_cotaskmem_pwstr, succeeded, uninitialize_com,
        ComPtr, IMMDeviceVtbl,
    },
    game_matching::clean_user_facing_process_name,
    valid_capture_dimensions,
    windows::wide_array_to_string,
    DetectedGame, GameDetection,
};
use crate::types::{RecordingSettings, VideoDimensions};

pub(super) const S_OK: HRESULT = 0;
pub(super) const IID_IAUDIO_SESSION_MANAGER2: GUID =
    GUID::from_u128(0x77aa99a0_1bd6_484f_8bc7_2c654c9a9b6f);
pub(super) const IID_IAUDIO_SESSION_CONTROL2: GUID =
...
#[derive(Clone)]
pub(super) struct AudioSessionProcess {
...
pub(super) struct AudioWindowContext {
    pub(super) process_ids: HashSet<u32>,
    pub(super) windows: HashMap<u32, WindowInfo>,
}

pub(super) struct GameWindowContext<'a> {
    pub(super) foreground_process_id: Option<u32>,
    pub(super) settings: &'a RecordingSettings,
    pub(super) candidates: Vec<GameDetection>,
}

#[repr(C)]
#[allow(non_snake_case)]
pub(super) struct IAudioSessionManager2Vtbl {
    base: IUnknown_Vtbl,
    GetAudioSessionControl: usize,
    GetSimpleAudioVolume: usize,
    pub(super) GetSessionEnumerator:
...
#[repr(C)]
#[allow(non_snake_case)]
pub(super) struct IAudioSessionEnumeratorVtbl {
    base: IUnknown_Vtbl,
    pub(super) GetCount: unsafe extern "system" fn(*mut c_void, *mut i32) -> HRESULT,
    pub(super) GetSession: unsafe extern "system" fn(*mut c_void, i32, *mut *mut c_void) -> HRESULT,
}
...
#[repr(C)]
#[allow(non_snake_case)]
pub(super) struct IAudioSessionControl2Vtbl {
    base: IUnknown_Vtbl,
    GetState: usize,
    ...
    pub(super) GetProcessId: unsafe extern "system" fn(*mut c_void, *mut u32) -> HRESULT,
    pub(super) IsSystemSoundsSession: unsafe extern "system" fn(*mut c_void) -> HRESULT,
...
pub(super) fn active_audio_sessions() -> HashMap<u32, AudioSessionProcess> {
    unsafe {
...
unsafe fn stream_size(stream: *mut c_void) -> Option<usize> {
    ...
    if !succeeded(((*stream_vtbl).Stat)(stream, &mut stat, STATFLAG_NONAME)) {
        return None;
    }
...
pub(super) fn process_executable(path: Option<&str>) -> Option<String> {
...
pub(super) unsafe fn is_capturable_application_window(hwnd: HWND) -> bool {
    if hwnd.is_null()
        || IsWindow(hwnd) == 0
...
pub(super) fn looks_like_audio_application(
    title: &str,
...
unsafe fn file_version_translations(data: &[u8]) -> Vec<VersionTranslation> {
    ...
    let mut len = 0u32;
    if VerQueryValueW(data.as_ptr().cast(), query.as_ptr(), &mut buffer, &mut len) == 0
        || buffer.is_null()
        || len < 4
    {
        return Vec::new();
...
pub(super) unsafe fn window_text(hwnd: HWND) -> Option<String> {
...
pub(super) unsafe fn window_class(hwnd: HWND) -> Option<String> {
...
pub(super) unsafe fn process_path(process_id: u32) -> Option<String> {
...
pub(super) unsafe fn fullscreen_monitor_dimensions(hwnd: HWND) -> Option<VideoDimensions> {
...
pub(super) fn refresh_capture_metadata(game: &mut DetectedGame) {
    unsafe {
...
pub(super) unsafe fn window_dimensions(hwnd: HWND) -> Option<VideoDimensions> {
    let mut rect = RECT::default();
...
pub(super) unsafe fn window_hdr_enabled(hwnd: HWND) -> bool {
...
unsafe fn display_path_source_name(path: &DISPLAYCONFIG_PATH_INFO) -> Option<String> {
    ...
    Some(wide_array_to_string(&source_name.viewGdiDeviceName)).filter(|value| !value.is_empty())
}
...
pub(super) fn rect_dimensions(rect: &RECT) -> Option<VideoDimensions> {
