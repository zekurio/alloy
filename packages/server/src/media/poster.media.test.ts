...
test("extractPoster skips a black lead-in and returns a resized JPEG with BlurHash", async () => {
...
test("extractPoster handles grayscale decoded frames", async () => {
  const workDir = await mkdtemp(join(tmpdir(), "alloy-poster-test-"))
  try {
...
test("extractPoster accepts an explicit uniform frame when allowed", async () => {
  const workDir = await mkdtemp(join(tmpdir(), "alloy-poster-test-"))
  try {
...
test("extractPoster returns null when every candidate is uniform", async () => {
  const workDir = await mkdtemp(join(tmpdir(), "alloy-poster-test-"))
  try {
