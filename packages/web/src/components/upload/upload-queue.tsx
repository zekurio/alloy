          ...
          {isUnavailable && queue.length === 0
            ...
            : isLoading && queue.length === 0
              ...
              : queue.length === 0
                ...
                : tx("{count} {label}", {
                    count: queue.length,
                    label: queue.length === 1 ? tx("item") : tx("items"),
                  })}
        </span>
      </header>
      ...
      <div className="-mx-1 flex flex-col">
        {isUnavailable && queue.length === 0 ? (
          <div className="border-border mx-1 flex flex-col items-center justify-center gap-2 rounded-md border px-6 py-8 text-center">
            ...
            <div className="space-y-1">
              <p className="text-foreground text-sm font-medium">
                {tx("Transfer status unavailable")}
              </p>
              <p className="text-foreground-muted text-xs font-semibold">
                {tx("Reopen transfers after the connection recovers.")}
              </p>
            </div>
          </div>
        ) : isLoading && queue.length === 0 ? (
          ...
          <div className="border-border mx-1 flex flex-col items-center justify-center gap-1 rounded-md border border-dashed px-6 py-8 text-center">
            <p className="text-foreground text-sm font-medium">
              {tx("Nothing in the queue")}
            </p>
            <p className="text-foreground-muted text-xs font-semibold">
              {tx("Uploads and downloads will show up here.")}
            </p>
          </div>
        ) : (
  ...
  return (
    <div className="mt-2 flex items-center justify-between px-2 pb-0.5 first:mt-0">
      <span className="text-foreground-faint flex items-center gap-1.5 text-[0.6875rem] font-semibold tracking-wide uppercase">
        {kind === "upload" ? tx("Uploads") : tx("Downloads")}
      ...
      {kind === "upload" && syncPaused != null && onToggleSyncPause ? (
        <Button
          variant="ghost"
          size="icon-sm"
          aria-label={syncPaused ? tx("Resume sync") : tx("Pause sync")}
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1.5 pr-9">
          <div className="flex min-w-0 items-baseline gap-2">
            <span className="text-foreground truncate text-sm font-semibold tracking-[var(--tracking-tight)]">
              {item.title}
            </span>
          </div>
  ...
  return (
    <div
      ...
      className={cn(
        ...
        // Keep the composited blurhash canvas inside rounded corners.
        "isolate",
      )}
    >
  ...
  if (status === "uploading" || status === "downloading") {
    return (
      <Button
        ...
        aria-label={
          status === "downloading"
            ? tx("Cancel download of {title}", { title })
            : tx("Cancel upload of {title}", { title })
        }
...
const STATUS_LABELS: Record<QueueItemStatus, string> = {
  ...
  downloading: tx("Download"),
  downloaded: tx("Saved locally"),
  failed: tx("Failed"),
}
