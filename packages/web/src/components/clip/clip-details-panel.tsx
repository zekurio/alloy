...
export function ClipDetailsPanel({
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-6">
      {/*
       * Clip metadata is pinned and only the recommendations scroll. Both the
       * header and the description are user-written, so this block caps its
       * height: past the cap it scrolls inside itself instead of pushing the
       * list out of reach.
       */}
      <div className="max-h-[calc(100%-10rem)] shrink-0 overflow-y-auto overscroll-contain px-5 pt-5">
        <ClipMeta
          clipId={row.id}
          status={row.status}
