...
Deno.test("liveCodecsFromSupport preserves AV1, HEVC, H.264 priority", () => {
...
Deno.test("liveCodecsFromSupport returns empty list when no probe matches", () => {
  const codecs = liveCodecsFromSupport({
    canPlayType: () => "",
...
Deno.test("liveCodecsFromSupport treats HEVC hvc1 and hev1 as equivalent support", () => {
  const codecs = liveCodecsFromSupport({
