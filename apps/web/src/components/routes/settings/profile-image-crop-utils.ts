...
export function cropGeometry({
  ...
  const cropFrame = getCropFrame(stage, aspect, mode, containedImageBox)
  const imageBox = image ? getImageBox(image, stage, zoom) : null
  return { containedImageBox, cropFrame, imageBox }
}
