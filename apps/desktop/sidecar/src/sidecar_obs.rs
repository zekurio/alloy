...
unsafe fn configure_game_capture_source(
    ...
    obs.set_bool(data, "capture_overlays", true)?;
...
fn source_kind(settings: &RecordingSettings, game: Option<&DetectedGame>) -> OutputSourceKind {
    if settings.record_desktop && game.is_none() {
        OutputSourceKind::Display
    } else if settings.record_desktop && game.is_some() {
...
fn source_kind_for_focus(settings: &RecordingSettings, focused: bool) -> OutputSourceKind {
    if settings.record_desktop && !focused {
        OutputSourceKind::Display
    } else {
        OutputSourceKind::Game
    }
