...
impl Recorder {
    ...
    fn start_replay_buffer(&mut self) -> Result<(), String> {
        ...
        let path = saved_recording_path(&output_folder, "Clips", Some(&game.game));
    ...
    fn tick(&mut self) {
        ...
        let ended_game = self.observe_game(detect_game_activity(active_game.as_ref()));
        ...
        if settings.enabled && self.mode == RecordingMode::Idle && self.active_game.is_some() {
