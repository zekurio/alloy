...
export interface UploadTicket {
  ...
  method: "PUT" | "POST"
  /**
   * Headers the browser must echo on the upload. Always includes
   * `Content-Type` bound to the value the ticket was minted for — the
   * fs upload route compares against the HMAC payload and 400s on a
   * mismatch, and S3 signs the header into the presigned URL. Leaving
   * this to `Blob.type` would break on MIME dialects the browser
   * reports differently from what we normalise to (e.g. Firefox's
   * `video/matroska` vs our `video/x-matroska`).
   */
...
/**
 * Extra encoded renditions live next to the default `video.mp4` output.
 * The variant id is generated by the server (`480p`, `720p`, …), so a
 * simple conservative sanitiser is enough to keep the on-disk filename
 * well-formed without letting arbitrary route input shape the path.
 */
